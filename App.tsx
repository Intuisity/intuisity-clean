...
import { backendUserInsightsCsvUrl, loadAdminSecret, loadBackendAdminReport, saveAdminSecret, syncDailyAnswers, syncProfile } from "./src/services/backendApi";
...
function AccountAccess({ onAuthenticated }: { onAuthenticated: (profile: UserProfile) => void }) {
  ...
  return (
    <SafeAreaView style={styles.accountFormScreen}>
      <ScrollView contentContainerStyle={styles.accountFormContent}>
        <Image
...
function getGoogleClientId() {
  ...
  return env.EXPO_PUBLIC_GOOGLE_CLIENT_ID || env.GOOGLE_CLIENT_ID || "";
}
...
async function signInWithGoogle(): Promise<GoogleProfile> {
  ...
  if (!clientId) {
    throw new Error("Google sign-in needs a Google Client ID added in Vercel first.");
  }
...
function AdminDashboard() {
  ...
  const localReport = loadAdminAnalyticsReport();
  ...
  const summaryMetrics = [
    ...
    { icon: "time-outline" as const, label: "Total time", value: formatDuration(report.totalTimeMs) },
    { icon: "timer-outline" as const, label: "Average visit", value: formatDuration(report.averageSessionMs) }
  ];
  const downloadUserInsights = () => {
    const opener = (globalThis as unknown as { open?: (url: string, target?: string) => void }).open;
    if (opener) {
      opener(backendUserInsightsCsvUrl(adminSecret), "_blank");
    } else {
      Alert.alert("CSV download", `Open this backend link to download the CSV: ${backendUserInsightsCsvUrl(adminSecret)}`);
    }
  };

  useEffect(() => {
    let active = true;
    loadBackendAdminReport(adminSecret).then((nextReport) => {
      if (!active) return;
      if (nextReport) {
  ...
  return (
    <View>
      ...
      {report.userInsights?.length ? report.userInsights.slice(0, 8).map((user) => (
        <View key={user.email} style={styles.adminUserCard}>
          ...
          <View style={styles.adminUserStatsGrid}>
            ...
            <View style={styles.adminUserStat}>
              <Text style={styles.adminUserStatLabel}>Most time</Text>
              <Text style={styles.adminUserStatValue}>{user.mostTimeModule}</Text>
              <Text style={styles.adminFeedbackMeta}>{formatDuration(user.mostTimeMs)}</Text>
            </View>
          </View>
      ...
      {report.moduleSummaries.length ? report.moduleSummaries.map((module) => (
        <View key={module.moduleLabel} style={styles.adminModuleRow}>
          <View style={styles.adminModuleTopline}>
            ...
            <Text style={styles.adminValue}>{formatDuration(module.totalMs)}</Text>
          </View>
          <View style={styles.adminTrack}>
            <View
              style={[
                styles.adminTrackFill,
                { width: `${Math.min(100, Math.max(8, Math.round((module.totalMs / Math.max(report.totalTimeMs, 1)) * 100)))}%` }
              ]}
            />
          </View>
          <Text style={styles.adminModuleMeta}>
            {module.visits} visits · {formatDuration(module.averageMs)} average
          </Text>
        </View>
      )) : (
        <View style={styles.adminEmptyCard}>
