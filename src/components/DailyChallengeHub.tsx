...
import { getAstrologyReading } from "../data/astrologyTips";
...
import { recordModuleTime } from "../services/adminAnalytics";
import { sendFriendInviteEmail, syncDailyResult, syncFriends, syncModuleFeedback } from "../services/backendApi";
...
const treasureWinMessages = [
  "Fantastic, you opened the chest",
  "Amazing, you unlocked the treasure",
  "Wonderful, you found the treasure",
  "Beautiful work, you opened the chest",
  "You did it, the treasure is yours"
];
...
export function DailyChallengeHub({ answers, homeRequestId = 0, isPremium, onLogout, onUpdateProfile, setAnswers, userProfile }: Props) {
  ...
  const astrologyReading = useMemo(
    () => getAstrologyReading(userProfile.birthdate, new Date(), userProfile.birthTime, birthLocation),
    [birthLocation, userProfile.birthTime, userProfile.birthdate]
  );
  ...
  useEffect(() => {
    ...
    async function loadPersonWithPortrait() {
      if (savedPersonChallengeRef.current?.completed) {
  ...
  }, [person.portraitUri, personPortraitUri, userProfile.email]);
    ...
    return (
      <View>
        <ChallengePageHeader
          ...
          subtitle="One color is hiding a beautiful real-world picture. Quiet your mind and choose the color that calls to you."
        />
        ...
        <View style={styles.knowingGuidance}>
          ...
          <Text style={styles.knowingGuidanceText}>
            Sit comfortably and quiet your mind. Take a slow breath, soften your focus, and notice which color feels as though it is holding the hidden picture. Trust the first gentle impression that comes to you.
          </Text>
        </View>
              ...
              >
                {hidesPhoto && (
                  <Image
                    ...
                    resizeMode="cover"
                    source={rewardImages[round].source}
    ...
    return (
      <View>
        <ChallengePageHeader
          ...
          title={lesson.title}
          subtitle="A simple daily task to build positivity, gratitude, kindness, or calm."
        />
        {priorLearningEntry && (
          <View style={styles.learningReminder}>
        ...
        <View style={styles.practiceCard}>
          <Text style={styles.practiceLabel}>Try this today</Text>
          <Text style={styles.practiceText}>{lesson.practice}</Text>
        </View>
        <View style={styles.reflectionCard}>
          <Text style={styles.practiceLabel}>Reflection</Text>
          <Text style={styles.practiceText}>{lesson.reflection}</Text>
        </View>
        <View style={styles.learningChallengeCard}>
          <Text style={styles.practiceLabel}>Your task for today</Text>
          <Text style={styles.practiceText}>
            You can use today's task above, or write a small positivity task of your own.
          </Text>
          <TextInput
            ...
            placeholder="Example: I will write a note to someone special today."
            placeholderTextColor="#9A93AA"
            style={[styles.journalInput, learningTaskSaved && styles.journalInputLocked]}
            textAlignVertical="top"
        ...
        {learningTaskSaved ? (
          ...
          <Pressable
            ...
            style={[styles.primaryButton, !learningChallenge.trim() && styles.disabledButton]}
          >
              ...
              <Pressable
                ...
                onPress={() => {
                  ...
                  if (nextSelections.length === personChoiceLimit) {
                    ...
                    saveTodaysPersonChallenge(userProfile.email, {
                      profileId: person.id,
      ...
      <View>
        ...
        <View style={styles.birthdateCard}>
          {hasSavedBirthDetails ? (
            ...
            >
              ...
              <View style={styles.menuCopy}>
                <Text style={styles.birthChartTitle}>Birth details saved</Text>
          ...
          {birthDetailsOpen && (
            <View>
              {hasSavedBirthDetails && (
                <Text style={styles.birthChartNote}>Make changes only if your birth details need updating.</Text>
              )}
              <TextInput
              ...
              >
                <Text style={styles.secondaryButtonText}>{birthDetailsSaved ? "Birth details saved" : "Save birth details"}</Text>
              </Pressable>
              {!birthdateReady && (
                <Text style={styles.birthChartNote}>Birthdate is needed first. Time and birthplace can be added for a fuller chart.</Text>
              )}
            </View>
          )}
        </View>
        ...
        {!astrologyReading ? (
          ...
          <View>
            ...
            {astrologyReading.fullChart ? (
              ...
              <View style={styles.fullChartCard}>
                <Text style={styles.fullChartTitle}>Full chart ready when birth details are complete</Text>
                <Text style={styles.fullChartSource}>
                  Add birth time and a recognized birth city to include Moon, Rising, Midheaven, houses, and aspects.
                </Text>
              </View>
            )}
  ...
  if (page === "social-prediction") {
    ...
    const treasureWon = Boolean(treasureWinText) && !treasureLost;
    ...
    const startTreasureChallenge = (mode: "friend" | "computer") => {
      if (mode === "friend" && selectedFriendPhones.length === 0) {
        setFriendPhoneError("Select or add at least one friend.");
        return;
      }
      if (mode === "friend") {
        const selectedFriends = savedFriends.filter((friend) => selectedFriendPhones.includes(getFriendKey(friend)));
        const emailFriends = selectedFriends.filter((friend) => friend.email);
        if (emailFriends.length > 0) {
          setFriendInviteStatus("Sending email invite...");
          Promise.all(emailFriends.map((friend) => sendFriendInviteEmail({
            ...
            .then(() => setFriendInviteStatus(`Email invite sent to ${emailFriends.length} ${emailFriends.length === 1 ? "friend" : "friends"}.`))
            .catch(() => setFriendInviteStatus("The challenge started, but the email could not be sent yet. Check Resend and Vercel settings."));
        } else {
          setFriendInviteStatus("Challenge created. Add a friend email to send live email invites.");
        }
      } else {
    ...
    const renderTreasurePlacementControls = () => {
      if (typeof (globalThis as any).document !== "undefined") {
        const webTokenStyle = (disabled: boolean, active: boolean): any => ({
          ...
          border: active ? "2px solid #F4B740" : "1px solid #DCCFF5",
          borderRadius: 8,
          color: "#30264C",
        ...
        const webSlotStyle = (locked: boolean, active: boolean): any => ({
          alignItems: "center",
          background: locked ? "#EDFFF6" : active ? "#FFFDF5" : "#FFFFFF",
          border: locked ? "2px solid #43C987" : active ? "2px dashed #F4B740" : "2px dashed #DAD3E8",
          borderRadius: 8,
          color: "#30264C",
        ...
        return React.createElement(
          ...
          React.createElement("div", { style: { color: "#706982", fontSize: 12, fontWeight: 900, marginBottom: 8, textTransform: "uppercase" } }, "Available treasures"),
          React.createElement(
            "div",
            ...
            treasureIcons.map((icon) => {
              ...
              return React.createElement(
                "div",
                {
                  ...
                  onDragStart: (event: any) => writeTreasureDrag(event, { icon, from: "palette" }),
          ...
          React.createElement("div", { style: { color: "#8A6B20", fontSize: 12, fontWeight: 900, lineHeight: "17px", marginBottom: 12, textAlign: "center" } }, "Drag treasures into the boxes below, or tap a treasure to place it in the next open box."),
          ...
          React.createElement("div", { style: { color: "#706982", fontSize: 12, fontWeight: 900, marginBottom: 8, textTransform: "uppercase" } }, "Your order"),
          React.createElement(
            "div",
            { style: { display: "flex", gap: 8, justifyContent: "center", marginBottom: 14 } },
            treasureGuess.map((icon, index) => {
              ...
              return React.createElement(
                "div",
                {
                  ...
                  draggable: Boolean(icon && !treasureLocked[index]),
                  ...
                  onDragStart: (event: any) => {
                    ...
                    writeTreasureDrag(event, { icon, from: "slot", index });
                  },
                  onDrop: (event: any) => {
                    event.preventDefault();
                  ...
                  style: webSlotStyle(treasureLocked[index], active)
                },
                icon || "+"
              );
            })
          ...
          React.createElement(
            "button",
            {
              disabled: treasureGuess.some((icon) => !icon),
              ...
              style: {
                alignItems: "center",
                background: treasureGuess.some((icon) => !icon) ? "#CFC8DA" : "#7555C7",
                border: 0,
                borderRadius: 8,
                color: "#FFFFFF",
                cursor: treasureGuess.some((icon) => !icon) ? "default" : "pointer",
                display: "flex",
                fontSize: 15,
                fontWeight: 900,
                gap: 8,
      ...
      return (
        <View>
          <Text style={styles.selectionCount}>Available treasures</Text>
                  ...
                  <View style={styles.treasureSlotRow}>
                    {row.map((icon, index) => {
                      ...
                      return (
                        ...
                        >
                          <Text style={styles.treasurePastSlotText}>{icon || "+"}</Text>
                        </View>
                      );
                    })}
                  </View>
          ...
          <Text style={styles.selectionCount}>Your order</Text>
          <View style={styles.treasureSlotRow}>
            {treasureGuess.map((icon, index) => (
                <Pressable
                  key={`treasure-slot-${index}`}
          ...
          <Pressable
            ...
            style={[styles.primaryButton, treasureGuess.some((icon) => !icon) && styles.disabledButton]}
          >
            <Ionicons color="#FFFFFF" name="key-outline" size={18} />
            <Text style={styles.primaryButtonText}>Try unlocking</Text>
          </Pressable>
        </View>
      );
    };
    ...
    return (
      <View>
        ...
        <View style={styles.opponentToggle}>
          ...
          <Pressable
            onPress={() => startTreasureChallenge("computer")}
          ...
          <View>
            ...
            <View style={styles.friendPhoneRow}>
              ...
              <Pressable
                ...
                disabled={!friendName.trim() || (!friendPhone.trim() && !friendEmail.trim())}
                onPress={addFriendPhone}
                style={[styles.addFriendButton, (!friendName.trim() || (!friendPhone.trim() && !friendEmail.trim())) && styles.disabledButton]}
              >
            ...
            <TextInput
              ...
              placeholder="Friend email for invite"
              placeholderTextColor="#9A93AA"
              style={styles.birthdateInput}
              value={friendEmail}
            />
        ...
        {!treasureStarted || treasureWinText ? (
          <Pressable
          ...
          >
            ...
            <Text style={styles.primaryButtonText}>
              {treasureWinText ? "Play again" : opponent === "friend" ? "Create friend treasure challenge" : "Start computer game"}
            </Text>
          </Pressable>
        ) : (
        ...
        {remotePhase === "sense" ? (
          <View>
            ...
            <View style={styles.drawingActions}>
              <Pressable onPress={() => setDrawingPoints([])} style={styles.secondaryButton}>
                <Ionicons color="#FFFFFF" name="trash-outline" size={17} />
                <Text style={styles.secondaryButtonText}>Clear drawing</Text>
              </Pressable>
              <Pressable onPress={() => setRemotePhase("choose")} style={styles.primaryButton}>
...
function BirthTimePicker({ onChange, value }: { onChange: (value: string) => void; value: string }) {
  ...
  return (
    <View style={styles.birthTimePicker}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.birthTimeScroller}>
        {birthTimeHours.map((hour) => (
          <Pressable
            accessibilityLabel={`Select birth hour ${hour}`}
            key={hour}
            onPress={() => updateTime({ hour })}
            style={[styles.birthTimeOption, selected.hour === hour && styles.birthTimeOptionSelected]}
          >
            <Text style={[styles.birthTimeOptionText, selected.hour === hour && styles.birthTimeOptionTextSelected]}>{hour}</Text>
          </Pressable>
        ))}
      </ScrollView>
      <Text style={styles.birthTimePickerLabel}>Minute</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.birthTimeScroller}>
        {birthTimeMinutes.map((minute) => (
          <Pressable
            accessibilityLabel={`Select birth minute ${minute}`}
            key={minute}
            onPress={() => updateTime({ minute })}
            style={[styles.birthTimeOption, selected.minute === minute && styles.birthTimeOptionSelected]}
          >
            <Text style={[styles.birthTimeOptionText, selected.minute === minute && styles.birthTimeOptionTextSelected]}>{minute}</Text>
          </Pressable>
        ))}
      </ScrollView>
      <View style={styles.birthTimePeriodRow}>
        {(["AM", "PM"] as const).map((period) => (
          <Pressable
            accessibilityLabel={`Select ${period}`}
            key={period}
            onPress={() => updateTime({ period })}
            style={[styles.birthTimePeriodButton, selected.period === period && styles.birthTimeOptionSelected]}
          >
            <Text style={[styles.birthTimeOptionText, selected.period === period && styles.birthTimeOptionTextSelected]}>{period}</Text>
          </Pressable>
        ))}
      </View>
    </View>
...
function makeTreasureWinMessage(triesUsed: number) {
  const phrase = shuffle(treasureWinMessages)[0] || "Fantastic, you opened the chest";
...
function getTodaysLesson() {
  const lessonIndex = Math.abs(hashString(`learning-${getDateKey()}`)) % dailyIntuitionLessons.length;
  return dailyIntuitionLessons[lessonIndex];
}
...
function loadTodaysPersonChallenge(email: string): DailyPersonChallenge | null {
  try {
    ...
    if (parsed?.date === getDateKey() && parsed.completed && savedProfile && isPhotographicPersonProfile(savedProfile)) return parsed;
  } catch {
    // Browser storage may be unavailable in private mode.
  }
  return null;
...
const styles = StyleSheet.create({
  ...
  remoteResultCompact: { marginBottom: 6, marginTop: 4, padding: 8 },
  remoteResultTitleCompact: { fontSize: 15 },
  remoteResultTextCompact: { fontSize: 12, lineHeight: 16, marginTop: 2 },
  remoteAnswerStage: { marginTop: 0 },
  remoteNextButtonCompact: { marginBottom: 4, minHeight: 40, padding: 8 },
  ...
  lessonCard: { backgroundColor: "#F8F7FC", borderColor: "#E7E3F2", borderRadius: 8, borderWidth: 1, marginBottom: 12, paddingHorizontal: 14, paddingTop: 12, paddingBottom: 6 },
  ...
  lessonPoint: { alignItems: "flex-start", flexDirection: "row", gap: 8, marginBottom: 8 },
  lessonPointText: { color: "#393149", flex: 1, fontSize: 14, lineHeight: 20 },
  practiceCard: { backgroundColor: "#EDFBFB", borderColor: "#BFE8E8", borderRadius: 8, borderWidth: 1, marginBottom: 12, padding: 16 },
  reflectionCard: { backgroundColor: "#FFF9E8", borderColor: "#F1DFA6", borderRadius: 8, borderWidth: 1, marginBottom: 16, padding: 16 },
  ...
  practiceLabel: { color: "#6544B8", fontSize: 13, fontWeight: "900", marginBottom: 6, textTransform: "uppercase" },
  practiceText: { color: "#393149", fontSize: 15, lineHeight: 22 },
  ...
  personPortraitFrame: { alignSelf: "center", borderColor: "#E7E3F2", borderRadius: 8, borderWidth: 1, height: 300, marginBottom: 16, maxWidth: 360, overflow: "hidden", width: "100%" },
  ...
  birthTimePicker: { backgroundColor: "#FFFFFF", borderColor: "#DAD3E8", borderRadius: 8, borderWidth: 1, marginBottom: 9, padding: 12 },
  birthTimeHeader: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
  birthTimeSelected: { color: "#008A94", fontSize: 12, fontWeight: "900", marginTop: -2 },
  birthTimeClearButton: { backgroundColor: "#F8F7FC", borderColor: "#DCCFF5", borderRadius: 999, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 7 },
  birthTimeClearText: { color: "#6544B8", fontSize: 12, fontWeight: "900" },
  birthTimePickerLabel: { color: "#706982", fontSize: 11, fontWeight: "900", marginBottom: 6, marginTop: 4, textTransform: "uppercase" },
  ...
  remotePictureGridCompact: { gap: 7, marginBottom: 4 },
  ...
  remotePictureChoiceCompact: { aspectRatio: 3.15 },
  ...
  treasureSplitLayout: { alignItems: "stretch", flexDirection: "row", flexWrap: "wrap", gap: 16, marginBottom: 12 },
  ...
  treasureToken: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#DCCFF5", borderRadius: 8, borderWidth: 1, cursor: "grab" as any, flex: 1, minHeight: 58, justifyContent: "center" },
  ...
  treasureTokenDisabled: { opacity: 0.28 },
  ...
  treasureAttemptList: { gap: 8, marginBottom: 12 },
  treasureAttemptBlock: { gap: 5 },
  treasureSlotRow: { flexDirection: "row", gap: 8, justifyContent: "center", marginBottom: 14 },
  treasureSlot: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#DAD3E8", borderRadius: 8, borderStyle: "dashed", borderWidth: 2, cursor: "grab" as any, flex: 1, minHeight: 58, justifyContent: "center" },
  ...
  remoteInstructionCard: { alignItems: "flex-start", backgroundColor: "#F8F5FF", borderColor: "#DCCFF5", borderRadius: 8, borderWidth: 1, flexDirection: "row", gap: 9, marginBottom: 12, padding: 12 },
  remoteInstructionText: { color: "#5D536A", flex: 1, fontSize: 13, fontWeight: "800", lineHeight: 19 },
  drawingPad: { backgroundColor: "#FFFFFF", borderColor: "#C5E1F3", borderRadius: 8, borderWidth: 2, height: 330, marginBottom: 12, overflow: "hidden", position: "relative" },
  ...
  drawingActions: { marginBottom: 10 },
